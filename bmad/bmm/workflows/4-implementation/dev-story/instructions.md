...
<workflow>
  ...
  <step n="4" goal="Run validations and tests">
    ...
    <action>Run pre-commit hooks manually to detect and fix issues before committing: Execute `npx lint-staged` or equivalent pre-commit command for the repository</action>
    <action>If pre-commit hooks fail (linting errors, formatting issues, etc.), analyze the error output and fix all issues automatically</action>
    <action>Re-run pre-commit hooks after fixes until they pass successfully</action>
    ...
    <check>If pre-commit hooks fail after 3 fix attempts → HALT and request guidance</check>
  </step>
  ...
  <step n="6" goal="Story completion sequence">
    ...
    <action>Run pre-commit hooks one final time to ensure all files pass validation: Execute `npx lint-staged` on all modified files</action>
    ...
    <check>If pre-commit hooks fail → STOP and fix issues before completing</check>
